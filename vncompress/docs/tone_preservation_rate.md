...
in [`vncompress/tone_aware/vietnamese_tones.py`](../tone_aware/vietnamese_tones.py)
as `compute_tone_preservation_rate()`; `ToneAwareCompressor` and
`CombinedCompressor` both call it rather than each recomputing the formula.
...
`tests/test_tone_preservation_rate.py::TestMajorityToneBaselineRate` for the
concrete comparison.

## Where TPR is reported

- `ToneAwareCompressor.compress()` and `CombinedCompressor.compress()` write
  `metadata['tone_preservation_rate']` on every `CompressionResult`.
- `vncompress/evaluation/metrics.py`'s `VCCBench.evaluate()` copies that
  value onto `CompressionMetrics.tone_preservation_rate` when present
  (`'tone_preservation_rate' in result.metadata`), and
  `_aggregate_metrics()` reports `mean_tone_preservation_rate` per
  method/ratio in the benchmark summary — see
  [`docs/benchmark.md`](../../docs/benchmark.md) for the result schema.
- Compressors that don't compute a tone signal (baselines like `none`,
  `random`, `llmlingua`, `snapkv`) simply have no `tone_preservation_rate`
  key in their metadata, so it's omitted from aggregation rather than
  reported as a misleading `0.0` or `1.0`.
